...
export default function NewQuestionPage() {
  ...
  const [sc_keyElements, setSc_keyElements] = useState('callSign')
  ...
  const buildQuestionData = (): Record<string, unknown> => {
    switch (category) {
      case 'readback':
        return { atcInstruction: rb_atcInstruction, incorrectReadback: rb_incorrectReadback, correctReadback: rb_correctReadback, errors: [], explanation: rb_explanation }
      case 'scenario':
        return { callSign: sc_callSign, flightPhase: sc_flightPhase, atcClearance: sc_atcClearance, correctResponse: sc_correctResponse, keyElements: sc_keyElements.split(',').map(s => s.trim()).filter(Boolean), hints: sc_hints.split('\n').map(s => s.trim()).filter(Boolean) }
      case 'jumbled':
        return { instruction: jm_instruction, correctOrder: jm_correctOrder.split(',').map(s => s.trim()).filter(Boolean), category: jm_category }
      case 'pronunciation':
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/questions', {
        ...
        body: JSON.stringify({ category, difficulty, is_active: isActive, question_data: buildQuestionData(), source: 'manual' }),
      })
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="card p-6 space-y-5">
        ...
        {category === 'scenario' && (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Key Elements (comma-separated)</label>
              <input value={sc_keyElements} onChange={e => setSc_keyElements(e.target.value)} className="input-field" placeholder="altitude, heading, squawk, callSign" />
            </div>
            <div>
        ...
        {category === 'jumbled' && (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Correct Word Order (comma-separated) <span className="text-red-500">*</span></label>
      ...
      <div className="flex gap-3">
        ...
        <button onClick={() => { setIsActive(true); handleSave() }} disabled={saving} className="btn-secondary">
